...
export const SupplierList = ({ suppliers = [], onSort, sortField, sortOrder }) => {
  ...
  // Get payment terms badge styling
  const getPaymentTermsBadgeStyles = (terms) => {
    const termsMap = {
      'Net 30': 'bg-blue-100 text-blue-700',
      'Net 60': 'bg-purple-100 text-purple-700',
      'Net 90': 'bg-orange-100 text-orange-700',
      'COD': 'bg-green-100 text-green-700',
      'Prepaid': 'bg-yellow-100 text-yellow-700',
    };
    return termsMap[terms] || 'bg-gray-100 text-gray-700';
  };
            ...
            {suppliers.map((supplier, index) => {
              return (
                ...
                >
                  ...
                  {/* Payment Terms Badge */}
                  <div className="w-[120px] px-3 flex items-center flex-shrink-0">
                    <span className={`${getPaymentTermsBadgeStyles(supplier.paymentTerms)} px-2 py-1 rounded text-[9px] font-bold font-['Poppins',sans-serif] uppercase`}>
