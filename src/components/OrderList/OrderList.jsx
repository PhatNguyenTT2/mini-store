...
export const OrderList = ({ orders = [], onStatusChange, onPaymentStatusChange, onSort, sortField, sortOrder, onEdit }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm">
      ...
      {/* Invoice Modal */}
      {viewDetailsModal && (() => {
        // Calculate values if not present (for old orders)
        const calculateInvoiceValues = () => {
          // Calculate subtotal from items if not present
          let calculatedSubtotal = parseFloat(viewDetailsModal.subtotal) || 0;
          if (!calculatedSubtotal && viewDetailsModal.items) {
            calculatedSubtotal = viewDetailsModal.items.reduce((sum, item) => {
              return sum + ((parseFloat(item.quantity) || 0) * (parseFloat(item.price) || 0));
            }, 0);
          }

          // Get discount percentage and calculate discount amount
          const discountPercentage = parseFloat(viewDetailsModal.discountPercentage) || 0;
          const discountType = viewDetailsModal.discountType || 'retail';

          // Calculate discount: if discount amount is stored, use it; otherwise calculate from percentage
          let discount = parseFloat(viewDetailsModal.discount) || 0;
          if (!discount && discountPercentage > 0) {
            discount = calculatedSubtotal * (discountPercentage / 100);
          }

          // Calculate values
          const shippingFee = parseFloat(viewDetailsModal.shippingFee) || 0;
          const subtotalAfterDiscount = calculatedSubtotal - discount;
          const tax = parseFloat(viewDetailsModal.tax) || (subtotalAfterDiscount * 0.1);
          const total = parseFloat(viewDetailsModal.total) || (subtotalAfterDiscount + shippingFee + tax);
            ...
            >
              ...
              <div className="overflow-y-auto max-h-[calc(90vh-180px)] p-8 bg-white" id="invoice-content">
                ...
                <div className="grid grid-cols-2 gap-4 mb-6 text-[13px]">
                  <div>
                    <p className="font-['Poppins',sans-serif]">
                      <span className="font-semibold text-blue-600">Order Number:</span> {viewDetailsModal.orderNumber || viewDetailsModal.id}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="font-['Poppins',sans-serif]">
                      <span className="font-semibold text-blue-600">Date:</span> {viewDetailsModal.date}
                    </p>
                  </div>
                </div>
                ...
                <div className="mb-6 space-y-2">
                  <p className="text-[14px] font-['Poppins',sans-serif]">
                    <span className="font-semibold text-blue-600">Customer Name:</span> {viewDetailsModal.customerName || viewDetailsModal.customer?.name || 'N/A'}
                  </p>
                  {viewDetailsModal.customer?.phone && (
                    <p className="text-[13px] font-['Poppins',sans-serif]">
                      <span className="font-semibold text-blue-600">Phone:</span> {viewDetailsModal.customer.phone}
                    </p>
                  )}
                  {viewDetailsModal.customer?.email && (
                    <p className="text-[13px] font-['Poppins',sans-serif]">
                      <span className="font-semibold text-blue-600">Email:</span> {viewDetailsModal.customer.email}
                    </p>
                  )}
                  <p className="text-[13px] font-['Poppins',sans-serif]">
                    <span className="font-semibold text-blue-600">Delivery Type:</span>
                    <span className={`ml-2 capitalize ${viewDetailsModal.deliveryType === 'delivery' ? 'text-emerald-600' : 'text-blue-600'}`}>
                      {viewDetailsModal.deliveryType || 'N/A'}
                    </span>
                  </p>
                  {viewDetailsModal.deliveryType === 'delivery' && viewDetailsModal.shippingAddress && (
                    <>
                      <p className="text-[13px] font-['Poppins',sans-serif]">
                        <span className="font-semibold text-blue-600">Shipping Street:</span> {viewDetailsModal.shippingAddress.street || 'N/A'}
                      </p>
                      <p className="text-[13px] font-['Poppins',sans-serif]">
                        <span className="font-semibold text-blue-600">Shipping City:</span> {viewDetailsModal.shippingAddress.city || 'N/A'}
                      </p>
                    </>
                  )}
                </div>
                ...
                <div className="mt-8 text-center">
                  ...
                  <p className="text-[14px] font-semibold font-['Poppins',sans-serif] text-blue-600">
                    {viewDetailsModal.user?.name || viewDetailsModal.user?.username || 'Admin'}
                  </p>
                </div>
              </div>
              ...
              <div className="px-6 py-4 border-t border-gray-200 bg-gray-50 flex justify-between items-center gap-3">
                ...
                <div className="flex gap-3">
                  <button
                    onClick={() => {
                      setViewItemsModal(viewDetailsModal);
